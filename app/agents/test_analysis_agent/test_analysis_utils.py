...
CRITICAL — Instance-layer Dockerfiles: If the Dockerfile starts with `FROM internal-swe-bench-...`, it is an instance-layer Dockerfile that MUST inherit from that pre-built base image. Do NOT instruct the WriteDockerfileAgent to change the base image to `python:*`, `ubuntu:*`, or any other public image. If the build fails with "pull access denied" for a `internal-swe-bench-...` image, the base image simply needs to be built locally first — this is an infrastructure issue, not a Dockerfile error. In that case, set `guidance_for_write_dockerfile_agent` to empty and do not suggest changing the base image.
